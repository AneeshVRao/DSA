# Practice Problems - 17 Bit Manipulation (Go)

If a problem demands `O(1)` extra space and involves duplicates or a missing
value, try XOR. Use `math/bits` in production code.
...
| 4 | Number of 1 Bits | Kernighan, or `bits.OnesCount`. | [LeetCode 191](https://leetcode.com/problems/number-of-1-bits/) |
...
| 7 | Reverse Bits | Shift out, shift in; compare with `bits.Reverse32`. | [LeetCode 190](https://leetcode.com/problems/reverse-bits/) |
...
- [ ] I know unary `^` is NOT in Go, and `&^` is AND NOT.
- [ ] I can explain `n & (n - 1)` and `n & -n`.
- [ ] I know `math/bits` and prefer it over hand-rolled loops in real code.
- [ ] I know shifting past the width gives 0 (not UB), but negative shifts panic.
