...
unsigned fromGray(unsigned gray) {
    unsigned n = gray;
    for (unsigned shift = 1; n >> shift; shift <<= 1) {
...
vector<unsigned> grayCodeSequence(int bits) {
    vector<unsigned> out(size_t(1) << bits);
    for (size_t i = 0; i < out.size(); i++) out[i] = toGray(unsigned(i));
    return out;
}
...
int main() {
    ...
    // --- Gray code ------------------------------------------------------------
    assert((grayCodeSequence(3) == vector<unsigned>{0, 1, 3, 2, 6, 7, 5, 4}));
    assert(toGray(0) == 0);
    assert((grayCodeSequence(0) == vector<unsigned>{0}));
    assert((grayCodeSequence(1) == vector<unsigned>{0, 1}));
    assert((grayCodeSequence(2) == vector<unsigned>{0, 1, 3, 2}));

    for (int bits = 1; bits <= 12; bits++) {
        vector<unsigned> sequence = grayCodeSequence(bits);

        // It is a permutation of 0..2^bits-1: nothing missing or doubled.
        assert(sequence.size() == (size_t(1) << bits));
        vector<char> seen(size_t(1) << bits, 0);
        for (unsigned value : sequence) {
            assert(!seen[value]);
            seen[value] = 1;
        }
        ...
        for (size_t i = 1; i < sequence.size(); i++) {
            assert(__builtin_popcount(sequence[i] ^ sequence[i - 1]) == 1);
        }

        // It is CYCLIC too - the last wraps to the first in one bit as well.
        assert(__builtin_popcount(sequence.back() ^ sequence.front()) == 1);

        // Round-trip.
        for (unsigned n = 0; n < (1u << bits); n++) assert(fromGray(toGray(n)) == n);

        // And agreement with the reflected construction.
        if (bits > 1) {
            vector<unsigned> smaller = grayCodeSequence(bits - 1);
            vector<unsigned> reflected = smaller;
            for (auto it = smaller.rbegin(); it != smaller.rend(); ++it) {
                reflected.push_back((1u << (bits - 1)) | *it);
            }
