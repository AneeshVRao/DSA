...
|-----------|------|-------|
| `gcdOf`, `lcmOf`, `extendedGcd` | `O(log min(a,b))` | `O(1)` |
| `modPow` | `O(log e)` | `O(1)` |
...
**`int64_t` overflows silently.** This is the whole difference from Python. Two
residues mod `10^9+7` multiply to `~10^18`, which *just* fits; two residues near
`2^62` do not, and the wrap-around produces a plausible wrong answer with no
warning. Miller-Rabin on a 64-bit `n` squares values right up against the limit,
so it needs a wider intermediate:
...
`__int128` is a GCC/Clang extension - not standard C++, but present on every
64-bit target. Without it you would need Russian-peasant multiplication
(`O(log b)` additions) or `long double` tricks.

**`std::gcd` and `std::lcm`** live in `<numeric>` since C++17. Use them in real
code; the hand-written versions here exist to show the algorithm, and the demo
cross-checks against the library.

**`vector<char>`, not `vector<bool>`, for sieves.** `vector<bool>` is the
notorious specialisation that packs eight entries per byte - it saves memory but
every write is a read-modify-write, and this loop is write-heavy. `vector<char>`
is measurably faster.

**Watch `i * i <= n` against overflow too.** For `n` near `INT64_MAX` the square
itself wraps. In this module the sieve loop uses `long long p` deliberately.

**`std::map` vs `std::unordered_map` for factorisations.** `map` is used here
because factor sets are tiny (at most 15 distinct primes below `2^64`) and
ordered iteration makes the output deterministic and comparable.
...
| File | Contents |
|------|----------|
| `number_theory.cpp` | All of the above, self-verifying |
...
`gcdOf` against `std::gcd`, `modPow` against a naive `O(e)` loop, the sieve
against trial division against Miller-Rabin on every `n` up to 2000, and `nCr`
against a Pascal's-triangle table.
